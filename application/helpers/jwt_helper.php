...
//		$keyid = $header->kid;
		...
		if ($verify) {
			...
			$CI =& get_instance();
			if ($signature != JWT::sign("$headb64.$bodyb64", $CI->config->item('jwt_key'), $header->alg,$payload)) {
	...
	public static function getTokenPayloadInfo($jwt)
	{
		
		$tks = explode('.', $jwt);
		list($headb64, $bodyb64, $cryptob64) = $tks;
		$bodyb64 = JWT::jsonDecode(JWT::urlsafeB64Decode($bodyb64)); 
		return $bodyb64;
	}
